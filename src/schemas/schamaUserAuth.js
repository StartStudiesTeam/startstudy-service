...
const phone_number_brazilian = joi.string().regex(/^(?:(\+55|55)?[1-9]{2}9?[6-9][0-9]{3}[0-9]{4})$/);
const phone_number_international = joi.string().regex(/^(?:\+[0-9] ?){6,14}[0-9]$/);
...
const schemaAuth = joi.object({
  ...
  phone_number: joi.alternatives().try(phone_number_brazilian, phone_number_international)
    .messages({
      'alternatives.types': 'O campo telefone precisa está em um formato válido',
      'alternatives.match': 'O campo telefone precisa está em um formato válido',
    }),
});
